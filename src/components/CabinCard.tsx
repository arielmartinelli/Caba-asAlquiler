...
export default function CabinCard({ cabin }: { cabin: CabinData }) {
  ...
  return (
    <div className="bg-white dark:bg-[#151c28] rounded-3xl overflow-hidden flex flex-col h-full border border-slate-200 dark:border-slate-800 shadow-md hover:shadow-2xl transition-all duration-300 group">
      ...
      <div className="relative h-64 w-full overflow-hidden bg-slate-100 dark:bg-slate-800">
        ...
        <button
          ...
          className="absolute top-4 right-4 p-2.5 rounded-full bg-white/90 dark:bg-slate-900/90 text-slate-800 dark:text-slate-200 shadow-md transition-all hover:scale-110"
        ...
        <div className="absolute top-4 left-4">
          <span className="px-3.5 py-1.5 rounded-full text-xs font-extrabold text-slate-900 dark:text-white bg-white/95 dark:bg-slate-900/95 backdrop-blur-md shadow-md border border-slate-200/80 dark:border-slate-800 flex items-center gap-1.5">
      ...
      <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
        <div className="space-y-2">
          <h3 className="text-xl font-extrabold text-slate-900 dark:text-white group-hover:text-rose-500 transition-colors leading-snug">
            {cabin.name}
          </h3>
          ...
          <p className="text-slate-700 dark:text-slate-300 text-xs line-clamp-2 leading-relaxed font-normal pt-1">
            {cabin.description}
          </p>
        </div>
        ...
        <div className="space-y-4 pt-3 border-t border-slate-200/80 dark:border-slate-800/80">
          <div className="flex flex-wrap gap-1.5">
            {cabin.amenities.slice(0, 3).map((amenity, idx) => (
              <span
                key={idx}
                className="text-[11px] bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-2.5 py-1 rounded-md text-slate-800 dark:text-slate-200 font-semibold flex items-center gap-1"
              >
                <CheckCircle2 className="w-3 h-3 text-emerald-600 dark:text-emerald-400 shrink-0" /> {amenity}
              </span>
            ))}
            {cabin.amenities.length > 3 && (
              <span className="text-[11px] bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-md text-slate-600 dark:text-slate-400 font-semibold">
          ...
          <div className="flex items-center justify-between pt-2">
            <div>
              <span className="text-[11px] text-slate-500 dark:text-slate-400 block font-bold uppercase tracking-wider">Noche desde</span>
